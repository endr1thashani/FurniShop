      ...
      <div className='container mx-auto'>
        <div className='flex justify-between items-center'>
          ...
          <nav className='hidden md:flex'>
            <ul className='md:flex md:gap-x-9'>
              {navigation.map((item,index)=>{
                return(
                  <li key={index}><a className='capitalize text-white hover:border-b transition-all' href={item.href}>{item.name}</a></li>
                )
              })}
            </ul>
          </nav>
          <div className={`${mobileNav ? 'left-0' : '-left-full'} md:hidden fixed bottom-0 w-full max-w-xs h-screen transition-all`}>
            <NavMobile closeMobileNav={closeMobileNav}/>
          </div>
        </div>
      </div>
    </header>
